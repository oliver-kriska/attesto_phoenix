defmodule AttestoPhoenix.AuthorizationServer.AuthorizationCodeCompletionTest do
  ...
  defmodule InjectedPrivateContextCodeStore do
    @moduledoc false
    @behaviour Attesto.CodeStore
    ...
    def take(code_hash) do
      case EctoCodeStore.take(code_hash) do
        {:ok, %{data: data} = record} ->
          private_context = Process.get(:injected_authorization_code_private_context)
          {:ok, %{record | data: Map.put(data, :attesto_phoenix_private_context, private_context)}}
    ...
    defdelegate record_access_token(family_id, jti, expires_at), to: EctoCodeStore
  end
  ...
  test "a host OAuth refusal may decline before invoking the continuation" do
    family_id = "family-oauth-refused"
    code = issue_code(family_id)

    refusal = %OAuthError{
      error: :access_denied,
      error_description: "subject authorization was revoked",
      status: 400
    }

    config =
      config(
        authorization_code_completion: fn _context, _continuation -> {:error, refusal} end,
        build_principal: fn _client, _subject, _scope ->
          flunk("a host refusal must not build a principal")
        end
      )

    assert {:error, ^refusal, _events} = Token.issue(config, code_request(config, code))
    assert_spent_without_completion(family_id)
  end
  ...
  test "a committed Repo.transaction wrapper is unwrapped" do
  ...
  test "a transaction wrapper that commits a failed continuation is rejected" do
    ...
    assert log =~ "committed a failed continuation"
    assert log =~ "completion writes may have escaped"
    row = authorization!(family_id)
    assert row.consumed_at
    refute row.consumed_success
    # The host committed instead of rolling back, so the library can reject and
    # diagnose the invalid wrapper but cannot retroactively undo its JTI write.
    assert is_binary(row.access_token_jti)
    assert TestRepo.aggregate(RefreshToken, :count) == 0
  end

  test "a callback that substitutes a different result after the continuation is refused" do
  ...
  test "an explicit JSON-null private context fails closed before the callback" do
    assert_malformed_private_context_refused(nil, "family-json-null-context")
  end

  test "a non-map private context fails closed before the callback" do
    assert_malformed_private_context_refused("not-a-map", "family-malformed-context")
  end
  ...
  test "an error remapped after a failed continuation is refused without a revocation hazard" do
  ...
  defp assert_malformed_private_context_refused(value, family_id) do
    Process.put(:injected_authorization_code_private_context, value)
    test_pid = self()

    code =
      issue_code(family_id, ["offline_access"], code_store: InjectedPrivateContextCodeStore)

    config =
      config(
        code_store: InjectedPrivateContextCodeStore,
        authorization_code_completion: fn _context, continuation ->
          send(test_pid, :malformed_context_callback_invoked)
          continuation.()
        end
      )
    ...
    refute log =~ inspect(value)
    refute_received :malformed_context_callback_invoked
    assert_spent_without_completion(family_id)
  after
    Process.delete(:injected_authorization_code_private_context)
  end
  ...
  defp assert_spent_without_completion(family_id) do
    ...
    # Table-wide, not filtered by the authorization provenance family: this
    # assertion must fail if any generation-0 refresh row escaped a rollback.
    # `setup` truncates the table before every test.
    assert TestRepo.aggregate(RefreshToken, :count) == 0
