defmodule AttestoPhoenix.Config do
  ...
  ### Optional callbacks
    ...
    * `:authorization_code_completion` - `(context, continuation -> result)`.
      ...
      dynamic scope and permits at most one invocation; a second, cross-process,
      ...
      `{:error, oauth_error}` result unchanged; the successful
      `{:ok, continuation_result}` wrapper from `Repo.transaction/1` is also
      accepted. If the continuation returns an error inside a transaction, the
      callback must roll that transaction back rather than commit the normal
      error tuple. Only stores participating in
