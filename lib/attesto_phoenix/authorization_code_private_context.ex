  ...
  def take(code_hash) when is_binary(code_hash) do
    ...
    case store.take(code_hash) do
      {:ok, %{data: data} = record} when is_map(data) ->
        case Map.fetch(data, @data_key) do
          :error ->
            Process.put(@captured_context_key, nil)
            {:ok, record}

          {:ok, private_context} when is_map(private_context) ->
            Process.put(@captured_context_key, private_context)
            {:ok, %{record | data: Map.delete(data, @data_key)}}

          {:ok, _malformed_or_json_null} ->
            Process.put(@captured_context_key, :invalid)
            {:ok, %{record | data: Map.delete(data, @data_key)}}
        end
